...
{
public:
	virtual int get_intersecting_surfaces(const model_t* model, const Math::CVector& vCenter, const float radius,
		const bool bOnlyVisibleSurfaces, surf_info* pInfos, const int nMaxInfos) = 0;
	virtual Math::CVector get_light_for_point(const Math::CVector& pos, bool bClamp) = 0;
	virtual i_material* trace_line_material_and_lighting(const Math::CVector& start, const Math::CVector& end, Math::CVector& diffuseLightColor,
		Math::CVector& baseColor) = 0;
	...
	virtual void get_viewangles(Math::CVector& va) = 0;
	virtual void set_viewangles(Math::CVector& va) = 0;
	...
	virtual int is_box_visible(const Math::CVector& mins, const Math::CVector& maxs) = 0;
	virtual int is_box_in_view_cluster(const Math::CVector& mins, const Math::CVector& maxs) = 0;
	virtual bool cull_box(const Math::CVector& mins, const Math::CVector& maxs) = 0;
	virtual void sound_extra_update(void) = 0;
	virtual const char* get_game_directory(void) = 0;
	virtual const Math::VMatrix& world_to_screen_matrix() = 0;
	virtual const Math::VMatrix& world_to_view_matrix() = 0;
	...
	virtual void compute_dynamic_lighting(const Math::CVector& pt, const Math::CVector* pNormal, Math::CVector& color) = 0;
	virtual void get_ambient_light_color(Math::CVector& color) = 0;
	...
	virtual bool does_box_touch_area_frustum(const Math::CVector& mins, const Math::CVector& maxs, int iArea) = 0; // 60
	...
	virtual void compute_lighting(const Math::CVector& pt, const Math::CVector* pNormal, bool bClamp, Math::CVector& color,
		Math::CVector* pBoxColors = nullptr) = 0;
	virtual void activate_occluder(int nOccluderIndex, bool bActive) = 0;
	virtual bool is_occluded(const Math::CVector& vecAbsMins, const Math::CVector& vecAbsMaxs) = 0; // 74
	...
	virtual void debug_draw_phys_collide(const c_phys_collide* pCollide, i_material* pMaterial, const Math::matrix3x4_t& transform,
		void* color) = 0; //79
	...
	virtual int is_skybox_visible_from_point(const Math::CVector& vecPoint) = 0;
	...
	virtual Math::CVector get_light_for_point_fast(const Math::CVector& pos, bool bClamp) = 0;
	...
	virtual void compute_lighting_cube(const Math::CVector& pt, bool bClamp, Math::CVector* pBoxColors) = 0;
	...
	virtual void solid_moved(class i_client_entity* pSolidEnt, class i_collideable* pSolidCollide,
		const Math::CVector* pPrevAbsOrigin, bool accurateBboxTriggerChecks) = 0;
	virtual void trigger_moved(class i_client_entity* pTriggerEnt, bool accurateBboxTriggerChecks) = 0;
	virtual void compute_leaves_connected(const Math::CVector& vecOrigin, int nCount, const int* pLeafIndices, bool* pIsConnected)
		= 0;
	...
	virtual void sphere_paint_surface(const model_t* model, const Math::CVector& location, unsigned char chr, float fl1,
		float fl2) = 0;
	virtual bool has_paintmap(void) = 0;
	virtual void enable_paintmap_render() = 0;
	//virtual void                TracePaintSurface( const model_t *model, const Math::CVector& position, float radius, CUtlMath::CVector<Color>& surfColors ) = 0;
	virtual void sphere_trace_paint_surface(const model_t* model, const Math::CVector& position, const Math::CVector& vec2, float radius,
		/*CUtlMath::CVector<unsigned char, CUtlMemory<unsigned char, int>>*/
		int& utilVecShit) = 0;
